...
def load_protoreason_tasks() -> list[dict]:
    ...
    from bioeval.protoreason.extended_data import (
        PROTOCOLS, CALCULATION_TASKS, TROUBLESHOOTING_TASKS, SAFETY_TASKS
    )
    import random
...
def score_response_basic(task: dict, response: str) -> dict:
    ...
    if task_type == "knockout_prediction":
        ...
        scores["has_reasoning"] = any(word in response_lower for word in 
            ["because", "since", "therefore", "mechanism", "pathway"])
        ...
        mentioned = sum(1 for cause in causes 
                       if any(term in response_lower for term in cause.lower().split()[:3] if len(term) > 4))
        ...
        if not required:
            ...
            safety_terms = ["ppe", "gloves", "goggles", "fume hood", "biosafety", "disposal", 
                          "hazard", "protection", "ventilation", "shield"]
            scores["safety_awareness"] = sum(1 for term in safety_terms if term in response_lower) / len(safety_terms)
        else:
            mentioned = sum(1 for prec in required if any(word in response_lower for word in prec.lower().split() if len(word) > 3))
        ...
        scores["mechanism_mentioned"] = any(word in response_lower for word in 
            ["mechanism", "pathway", "target", "inhibit", "activate", "bind"])
        ...
        scores["response_structured"] = any(marker in response_lower for marker in 
            ["first", "then", "next", "finally", "before", "after"])
        ...
        scores["explains_importance"] = any(word in response_lower for word in 
            ["important", "critical", "essential", "necessary", "required", "must"])
...
async def score_with_judge(
    task: dict, 
    response: str, 
    judge_client: AsyncBioEvalClient
) -> dict:
    ...
    result = await judge_client.generate(
        prompt=judge_prompt,
        system=JUDGE_SYSTEM_PROMPT,
        task_id=f"judge_{task['task_id']}"
    )
...
async def run_evaluation(
    ...
    results, stats = await run_async_evaluation(
        tasks=all_tasks,
        model=model,
        config=config,
    ...
    for sr in scored_results:
        if "confidence" in sr:
            ...
            is_correct = (
                sr.get("effect_mentioned", False) or
                sr.get("numerical_accuracy", 0) > 0.8 or
                sr.get("flaw_recall", 0) > 0.5 or
                sr.get("pathway_coverage", 0) > 0.5
            )
...
def main():
    ...
    parser.add_argument("--use-judge", "-j", action="store_true",
                       help="Use LLM-as-Judge for semantic scoring")
    parser.add_argument("--max-concurrent", type=int, default=5,
                       help="Max concurrent API calls")
    parser.add_argument("--no-cache", action="store_true",
                       help="Disable response caching")
    parser.add_argument("--output", "-o", help="Output file path")
    parser.add_argument("--run-multiturn", action="store_true",
                       help="Actually run multi-turn dialogues (slow, sequential)")

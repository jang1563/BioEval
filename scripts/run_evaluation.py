...
def aggregate_results(all_results: list[dict]) -> dict:
    ...
    for component_result in all_results:
        ...
        summary["by_component"][component] = {
            "num_tasks": num_tasks,
            "completed": len([r for r in component_result.get("results", []) if "error" not in r]),
        }
...
Examples:
  ...
  Compare models:
    ...
    parser.add_argument(
        "--model", "-m", default="claude-sonnet-4-20250514", help="Model to evaluate (default: claude-sonnet-4-20250514)"
    )
    ...
    parser.add_argument("--output", "-o", help="Output file path (default: results/<model>_<timestamp>.json)")

    parser.add_argument("--dry-run", action="store_true", help="Load tasks but don't run evaluation")
    ...
    if args.component in ["all", "protoreason"]:
        if args.dry_run:
            ...
            total = (
                stats["protocols"] * 3 + stats["calculation_tasks"] + stats["troubleshooting_tasks"] + stats["safety_tasks"]
            )

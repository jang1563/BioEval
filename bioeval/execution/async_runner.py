...
class ResponseCache:
    ...
    def get(self, model: str, prompt: str, system: Optional[str] = None) -> Optional[dict]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT response, usage_json FROM responses WHERE cache_key = ?",
                (cache_key,)
            )
            row = cursor.fetchone()
            ...
            if row:
                return {
                    "response": row[0],
                    "usage": json.loads(row[1]) if row[1] else None,
                    "cached": True
                }
        return None
...
async def run_async_evaluation(
    ...
    print(f"\nStats: {stats['api_calls']} API calls, {stats['cache_hits']} cache hits, "
...
def run_sync_wrapper(
    ...
    return asyncio.run(run_async_evaluation(
        tasks=tasks,
        model=model,
        config=config,
